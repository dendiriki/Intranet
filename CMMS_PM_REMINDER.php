    ...
    foreach ($data1 as $val) {
		...
		$sql = "SELECT a.vc_workorder_no DOC_NO, a.vc_remarks DOC_DESC, b.vc_remarks ASET_DESC, 
				...
				SELECT a.vc_schedule_no DOC_NO, a.vc_schedule_desc DOC_DESC, b.vc_remarks ASET_DESC, 
					   TO_CHAR(a.dt_start_date,'DD-MON-YYYY') START_DATE, 'WORK ORDER IS NOT GENERATED YET' REMARKS
		...
		if(!empty($data2)){
			...
			foreach ($data2 as $val2) {
				...
				if ($currem == $lastrem) {
					...
					if ($currem == 'OPEN WORK ORDER') {
						$email_msg2 .= "<tr><th>No.</th><th>Doc. No.</th><th>PM Schedule/Work Order</th><th>Asset/Equipment</th><th>Expirated Date</th><th>Remarks</th></tr>";
					} else {
						$email_msg2 .= "<tr><th>No.</th><th>Doc. No.</th><th>PM Schedule/Work Order</th><th>Asset/Equipment</th><th>Start Date</th><th>Remarks</th></tr>";
					}
					$email_msg2 .= "<tr><td>$i.</td><td>".$val2["DOC_NO"]."</td><td>".$val2["DOC_DESC"]."</td><td>".$val2["ASET_DESC"]."</td><td>".$val2["START_DATE"]."</td><td>".$val2["REMARKS"]."</td></tr>";
					$lastrem = $currem;
				}
		...
		$recipient[0]["email"] = "hary.purwantoro@mittalsteel.com";
		*/
		$mail = new SendMail();
