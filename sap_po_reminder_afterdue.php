...
$sql = " select a.ebeln,
           ...
           (TRUNC(sysdate) - to_date(e.eindt, 'YYYYMMDD')) as countdate,
           mod(TRUNC(sysdate) - to_date(e.eindt, 'YYYYMMDD'), 3)
     ...
     where a.mandt = b.mandt
       ...
       and e.wemng <= 0
       and e.etenr = '0001'
       and to_date(e.eindt, 'YYYYMMDD') < TRUNC(sysdate)
       and b.elikz = ' '
       and a.frgke in ('G','R')
     ...
     group by a.ebeln,
              ...
              to_date(e.eindt, 'YYYYMMDD')
    having mod(TRUNC(sysdate) - to_date(e.eindt, 'YYYYMMDD'), 3) = 0
     order by 1 asc";
...
if(!empty($data1)){
	...
	$email_msg1 .= "Please see the delivery date of the order is already expired ".$val["COUNTDATE"]." days over.<br><br>";
	$email_msg1 .= "Kindly take necessary action from your end and dispatch the material at the earliest.<br><br>";
	$email_msg1 .= "<table>";
	...
	$email_msg1 .= "</table><br>";
        ...
        $mail->sendTheMailPU("PO Delivery Reminder III", $email_msg, $recipient);
